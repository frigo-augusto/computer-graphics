...
void CV::point(float x, float y, float z, float d, bool perspectiveActive)
{
   glBegin(GL_POINTS);
   if (perspectiveActive)
   {
      if(z > 0){
         glVertex2d(x * d / z, y * d / z);
      }
...
void CV::point(Vector3 p, float d, bool perspectiveActive)
{
   glBegin(GL_POINTS);
   if (perspectiveActive)
   {
      if(p.z > 0)
         glVertex2d(p.x * d / p.z, p.y * d / p.z);
   }
   else
   {
      glVertex2d(p.x, p.y);
...
void CV::point(std::list<Vector3> &points)
{
   glBegin(GL_POINTS);
   for (auto p : points)
   {
...
void CV::line(Vector3 p1, Vector3 p2, float d, bool perspectiveActive)
{
   glBegin(GL_LINES);
   if (perspectiveActive)
   {
      if(p1.z > 0 && p2.z > 0){
         glVertex2d(p1.x * d / p1.z, p1.y * d / p1.z);
         glVertex2d(p2.x * d / p2.z, p2.y * d / p2.z);
      }
   }
   else
   {
